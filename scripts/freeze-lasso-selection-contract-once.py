...
MARKER = "## Lasso Selection / Lasso Fill / Lasso Eraser semantic and implementation contract — 2026-09-05 — AUTHORITATIVE"
...
if MARKER not in memo:
    memo_path.write_text(memo.rstrip() + BLOCK + "\n")

progress_path = Path("IMPLEMENTATION_PROGRESS.md")
progress = progress_path.read_text()
for index in range(1, 17):
    prefix = f"M8E-{index:03d} "
    matches = [line for line in progress.splitlines() if line.startswith(prefix)]
    if len(matches) != 1:
        raise SystemExit(f"M8E progress anchor missing/duplicate: {prefix}")
    old = matches[0]
    desired = old.rsplit(":", 1)[0] + ":未完了"
    if old != desired:
        progress = progress.replace(old, desired, 1)

for old, new in (
    ("M8E-検査 M8E内部検査:仮完了", "M8E-検査 M8E内部検査:未完了"),
    ("PUB-M8E M8Eユーザー確認用Preview公開:完了", "PUB-M8E M8Eユーザー確認用Preview公開:未完了"),
):
    if old in progress:
        progress = progress.replace(old, new, 1)
    elif new not in progress:
        raise SystemExit(f"M8E gate anchor missing: {new}")

gate = "USER-M8E M8E UI/UX/Visualユーザー確認PASS:未完了"
if gate not in progress:
    raise SystemExit("USER-M8E must remain pending")

note = "再開メモ: M8Eは2026-09-05のLasso Selection再監査でproduction-visible上層を再構築対象と判定。現行m8-selection-launcherの矩形AABB表示・combine mode bypass・source-token中心検査は不採用。正本のLasso Selection/Lasso Fill/Lasso Eraser authoritative contractに従い、gesture controller / coverage contour presenter / launcher command surfaceを分離して置換する。M7Aのselection coverage/combine/modifier等の正しい下層は保持し、M8FへはUSER-M8E PASSまで進まない。"
if note not in progress:
    progress = progress.replace(gate, gate + "\n" + note, 1)
