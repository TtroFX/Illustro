...
requireText(storageWorker, "storage-maintenance-worker-extension.js", 'maintenance extension is not in the production worker');
requireText(storageWorker, 'storageGrowthGuard.assertRawGrowth', 'raw object/tile growth is not quota-gated');
requireText(storageWorker, 'storageGrowthGuard.assertJsonGrowth', 'transaction/entity growth is not quota-gated');
requireText(storageWorker, 'storage.quota.unsafeGrowth', 'quota failures are not surfaced as data-safety errors');
...
requireText(history, 'DEFAULT_HISTORY_RETENTION_TRANSACTIONS = 1_000', '1000-transaction retention target missing');
requireText(historyStore, 'illustro.history-segment-envelope/1', 'history spill envelope missing');
requireText(coordination, "mode: 'exclusive', ifAvailable: true", 'exclusive Web Lock acquisition missing');
requireText(coordination, 'BroadcastChannel', 'BroadcastChannel propagation missing');
requireText(coordination, "mode: 'read-only', reason: 'locked-elsewhere'", 'second-tab read-only fallback missing');
...
requireText(quota, 'reserve(quota, QUOTA_WARNING_MIN_BYTES, 0.15)', 'warning reserve ratio missing');
requireText(quota, 'reserve(quota, QUOTA_CRITICAL_MIN_BYTES, 0.08)', 'critical reserve ratio missing');
...
requireText(growthGuard, 'STORAGE_TRANSACTION_WRITE_OVERHEAD_BYTES', 'transaction growth overhead missing');
...
requireText(garbageCollection, 'destructive: false', 'M2 GC foundation must remain non-destructive');
...
requireText(projectWorker, 'storageGrowthGuard.assertJsonGrowth', 'project lifecycle writes are not quota-gated');
requireText(historyWorker, 'storageGrowthGuard.assertJsonGrowth', 'history writes are not quota-gated');
