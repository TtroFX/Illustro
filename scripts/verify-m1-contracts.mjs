...
const sourceContracts = new Map([
  ...
  ['src/domain/state-boundaries.ts', ['createProjectMetadataState', 'createWorkspaceSessionStore', 'createDefaultWorkspaceStateV1']],
  ...
  ['src/domain/command-registry.ts', ['CommandRegistryV1', 'CommandTransactionId', 'parseCommandId']],
  ['src/domain/serialization.ts', ['serializeJson', 'deserializeJson', 'validateValueSchema', 'classifyVersionCompatibility']],
  ['src/domain/migration.ts', ['MigrationRegistryV1', 'createMigrationStep']],
  ['src/domain/reports.ts', ['createCompatibilityReport', 'createFidelityReport', 'createStructuredErrorRecord']],
  ['src/app/runtime-profile.ts', ['createRuntimeCapabilityProfile', 'probeArrayBufferTransferSupport']],
  ['src/domain/workspace-settings.ts', ['createDefaultWorkspaceStateV1', 'createDefaultUserSettingsV1', 'createUserSettingsStoreV1']],
...
for (const [relativePath, requiredTokens] of sourceContracts) {
  ...
  assert(!forbiddenStubPattern.test(content), `stub marker remains in M1 contract source: ${relativePath}`);
...
assert(main.includes("from './runtime-profile.js'"), 'runtime capability profile is not wired into app bootstrap');
assert(main.includes('createRuntimeCapabilityProfile('), 'runtime capability profile is not evaluated in production bootstrap');

const stateBoundaries = read('src/domain/state-boundaries.ts');
assert(stateBoundaries.includes("from './workspace-settings.js'"), 'workspace persistence schema is not connected to workspace/session state');
assert(stateBoundaries.includes('normalizeWorkspaceStateV1('), 'workspace/session store does not normalize persistent workspace state');

const commandRegistry = read('src/domain/command-registry.ts');
assert(commandRegistry.includes('document-transaction'), 'command registry lacks document transaction semantics');
assert(commandRegistry.includes('unavailable') && commandRegistry.includes('disabled'), 'command availability and disabled states are not distinct');

const memo = read('ILLUSTRO_DESIGN_MEMO.md');
assert(memo.includes('canonical Command Registry') || memo.includes('Canonical Command Registry'), 'canonical design memo no longer contains the command registry contract');
