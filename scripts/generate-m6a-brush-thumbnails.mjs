...
function parsePack(source) {
  ...
  const presets = match[1].trim().split('\n').map((line) => {
    const [id, name, category, behavior, size, hardness, spacing, flags, surface, tip, dynamics] = line.split('|');
    const d = dynamics.split(',').map(Number);
    return {
      id, name, category, behavior,
      size: Number(size), hardness: Number(hardness), spacing: Number(spacing), flags, surface, tip,
      textureStrength: d[2] ?? 0, angle: d[3] ?? 0, sizeJitter: d[6] ?? 0,
      positionJitter: d[7] ?? 0, densityJitter: d[8] ?? 0,
    };
  });
...
function pointsFor(preset, mode) {
  const seed = h32(`${GENERATION_ID}:${preset.id}:${mode}`);
  return Array.from({ length: 17 }, (_, i) => {
    ...
    return {
      ...
      y: 128 + Math.sin(t * Math.PI * 2.08 - 0.4) * 27 + (rnd(seed, i * 2 + 1) - 0.5) * (scatter ? 16 : 3),
      p,
      r: clamp(Math.sqrt(Math.max(1, preset.size)) * 2.55 * (preset.flags.includes('S') ? 0.3 + p * 0.86 : 1) * (1 + (rnd(seed ^ 0x51f15e5d, i) - 0.5) * preset.sizeJitter), 1.4, 29),
...
function representativeSvg(preset, mode) {
  const seed = h32(`${GENERATION_ID}:${preset.id}:${mode}:detail`);
  ...
  const soft = preset.hardness < 0.35 || preset.category === 'Airbrush' || preset.behavior === 'blur';
  const stroke = preset.behavior === 'erase' ? '#fbfcff' : '#26324b';
  const defs = soft ? '<defs><filter id="soft"><feGaussianBlur stdDeviation="2.8"/></filter></defs>' : '';
  const background = preset.behavior === 'erase'
  ...
  const segments = points.slice(0, -1).map((p, i) => {
    ...
    const alpha = ((p.a + q.a) / 2) * (textured ? 0.78 : 0.9);
    const dash = textured ? ` stroke-dasharray="${(3 + rnd(seed, i) * 8).toFixed(2)} ${(1.5 + preset.textureStrength * 7 + rnd(seed, i + 50) * 4).toFixed(2)}"` : '';
  ...
  const particles = scatter ? points.flatMap((p, i) => Array.from({ length: 3 + Math.round(preset.densityJitter * 4) }, (_, j) => {
    ...
    return `<circle cx="${(p.x + Math.cos(angle) * distance).toFixed(2)}" cy="${(p.y + Math.sin(angle) * distance).toFixed(2)}" r="${radius.toFixed(2)}" fill="${stroke}" opacity="${(0.28 + rnd(seed, i * 19 + j) * 0.44).toFixed(3)}"/>`;
  })).join('') : '';
  const blend = preset.behavior === 'smudge' || preset.behavior === 'blur'
    ? Array.from({ length: 5 }, (_, i) => `<path d="M30 ${82 + i * 23} C82 ${62 + i * 22},172 ${109 + i * 10},228 ${84 + i * 23}" fill="none" stroke="#53627c" stroke-width="${8 + i * 2}" stroke-linecap="round" opacity="${(0.12 + i * 0.055).toFixed(3)}"${preset.behavior === 'blur' ? ' filter="url(#soft)"' : ''}/>`).join('')
    : '';
...
async function writeAsset(name, content) {
  ...
  return { path: `assets/brush-thumbnails/${name}`, sha256: sha256(content), byteLength: Buffer.byteLength(content) };
}
...
for (const preset of presets) {
  const thumbnail = await writeAsset(`${preset.id}.svg`, representativeSvg(preset, 'representative'));
  const pressureLow = await writeAsset(`${preset.id}.pressure-low.svg`, representativeSvg(preset, 'low'));
  const pressureHigh = await writeAsset(`${preset.id}.pressure-high.svg`, representativeSvg(preset, 'high'));
  entries.push({ id: preset.id, name: preset.name, category: preset.category, behavior: preset.behavior, thumbnail, pressureLow, pressureHigh });
}
...
if (entries.some((entry) => entry.pressureLow.sha256 === entry.pressureHigh.sha256)) throw new Error('pressure reference differentiation failed');
...
const categoryPressureQa = `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="${categories.length * 256}" viewBox="0 0 512 ${categories.length * 256}">${categories.map((category, i) => { const entry = entries.find((candidate) => candidate.category === category); return `<image href="${entry.id}.pressure-low.svg" x="0" y="${i * 256}" width="256" height="256"/><image href="${entry.id}.pressure-high.svg" x="256" y="${i * 256}" width="256" height="256"/>`; }).join('')}</svg>\n`;
...
const manifest = {
  ...
  deterministicSeedBasis: 'sha256(generationId:presetId:pressureMode)',
...
const summary = {
  ...
  pressureReferencePairs: 48,
  pressureReferenceDistinctPairs: 48,
  manifestSHA256: sha256(manifestText),
  contactSheetSHA256: contactSheetAsset.sha256,
  categoryPressureQaSHA256: categoryPressureAsset.sha256,
  failureCount: 0,
};
await writeFile(new URL('m6a-brush-thumbnails.json', metaDir), `${JSON.stringify(summary, null, 2)}\n`, 'utf8');
