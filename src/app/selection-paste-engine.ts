...
import { createRasterLayer, type RasterLayerV1, type RasterTileReferenceV1 } from '../domain/layers.js';
import { tileBoundsForDocumentV1 } from '../gpu/sparse-tile-model.js';
import type { RasterMergePersistencePortV1, PreparedRasterMergeTileV1 } from './layer-raster-merge.js';
...
function defaultPasteLayerNameV1(snapshot: PaintProjectSnapshotV1): string {
  const names = new Set(Object.values(snapshot.document.layerTree.layers).map((layer) => layer.name));
...
export function selectionPasteEligibilityV1(
  ...
  if (transfer.tiles.length === 0) return unavailable('selection paste transfer contains no raster tiles');

  const keys = new Set<string>();
  try {
    for (const tile of transfer.tiles) {
      if (tile.payloadRef.length === 0) return unavailable('selection paste tile payload is missing');
    ...
    return unavailable(error instanceof Error ? error.message : 'selection paste tile grid is invalid');
  }
...
export async function prepareSelectionPasteV1(
  ...
  if (!eligibility.eligible) throw new Error(eligibility.reason ?? 'selection paste is unavailable');
  ...
  for (const tile of [...transfer.tiles].sort((left, right) => left.y - right.y || left.x - right.x)) {
