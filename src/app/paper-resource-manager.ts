...
import { BUILTIN_BRUSH_PAPER_RESOURCES_V1 } from '../domain/brush-schema.js';

export interface PaperResourceManagerSnapshotV1 {
  readonly schema: 'illustro.paper-resource-manager-state/1';
  readonly resourceCount: number;
  readonly loadedResourceCount: number;
}

export interface PaperResourceManagerV1 {
  readonly schema: 'illustro.paper-resource-manager/1';
  ...
  snapshot(): PaperResourceManagerSnapshotV1;
}
...
export function createPaperResourceManagerV1(
  loader: BuiltinSampledResourceLoaderV1,
  resources: readonly BuiltinSampledResourceDescriptorV1[],
): PaperResourceManagerV1 {
  const expectedAliases = new Set(BUILTIN_BRUSH_PAPER_RESOURCES_V1.map((resource) => resource.id));
  if (expectedAliases.size !== FINAL_SAMPLED_PAPER_COUNT_V1) {
    throw new RangeError(
      `canonical brush paper inventory must contain exactly ${FINAL_SAMPLED_PAPER_COUNT_V1} resources`,
    );
  }

  const papers = resources.filter(
    (resource) => resource.kind === 'grain' && resource.subtype === 'paper',
  );
  if (papers.length !== FINAL_SAMPLED_PAPER_COUNT_V1) {
    throw new RangeError(
      `paper resource manager requires exactly ${FINAL_SAMPLED_PAPER_COUNT_V1} resources`,
    );
  }

  const descriptors = new Map<string, BuiltinSampledResourceDescriptorV1>();
  for (const resource of papers) {
    if (!expectedAliases.has(resource.alias)) {
      throw new TypeError(`unknown canonical brush paper resource alias: ${resource.alias}`);
    }
    if (descriptors.has(resource.alias)) {
      throw new TypeError(`duplicate paper resource alias: ${resource.alias}`);
    }
    ...
    if (loaderDescriptor === null || !descriptorsMatch(resource, loaderDescriptor)) {
      throw new TypeError(`paper resource loader descriptor mismatch: ${resource.alias}`);
    }
    descriptors.set(resource.alias, resource);
  }
  for (const alias of expectedAliases) {
    if (!descriptors.has(alias)) {
      throw new TypeError(`missing canonical brush paper resource alias: ${alias}`);
    }
  }
  ...
  return Object.freeze({
    schema: 'illustro.paper-resource-manager/1' as const,
    descriptors(): readonly BuiltinSampledResourceDescriptorV1[] {
    ...
    async load(alias: string): Promise<LoadedBuiltinSampledResourceV1> {
      ...
      if (expected === null) throw new RangeError(`unknown built-in paper resource: ${alias}`);
      const loaded = await loader.load(alias);
      if (!descriptorsMatch(expected, loaded.descriptor)) {
        throw new Error(`loaded paper resource descriptor mismatch: ${alias}`);
      }
    ...
    snapshot(): PaperResourceManagerSnapshotV1 {
      return Object.freeze({
        schema: 'illustro.paper-resource-manager-state/1' as const,
...
export async function createProductionPaperResourceManagerV1(): Promise<PaperResourceManagerV1> {
  ...
  return createPaperResourceManagerV1(loader, manifest.resources);
}

let productionPaperResourceManagerPromise: Promise<PaperResourceManagerV1> | null = null;

export function startProductionPaperResourceManagerV1(): Promise<PaperResourceManagerV1> {
  if (productionPaperResourceManagerPromise === null) {
    productionPaperResourceManagerPromise = createProductionPaperResourceManagerV1();
  }
  return productionPaperResourceManagerPromise;
}

export function getProductionPaperResourceManagerV1(): Promise<PaperResourceManagerV1> | null {
  return productionPaperResourceManagerPromise;
}
