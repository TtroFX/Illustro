import {
  BRUSH_V1_SCHEMA, BUILTIN_BRUSH_GRAIN_RESOURCES_V1, BUILTIN_BRUSH_PAPER_RESOURCES_V1,
  createBaselineBrushPresetV1, normalizeBrushPresetV1, withBrushColorMixCanvasRatioV1,
  withBrushColorMixCarryAmountV1, withBrushColorMixDepositAmountV1, withBrushColorMixEnabledV1,
  withBrushColorMixPickupAmountV1, withBrushFollowStrokeRotationV1, withBrushGrainResourceIdV1,
  withBrushParameterValuesV1, withBrushPaperTextureResourceIdV1, withBrushPositionJitterV1,
  withBrushPressureOpacityEnabledV1, withBrushPressureSizeEnabledV1,
  withBrushRealtimeStabilizationAmountV1, withBrushRotationJitterV1, withBrushSizeJitterV1,
  withBrushDensityJitterV1, withBrushSprayDeviationV1, withBrushSprayEnabledV1,
  withBrushSprayParticleDensityV1, withBrushSprayParticleSizeRatioV1,
  withBrushSpraySpreadRadiusRatioV1, withBrushStrokeSpacingV1, withBrushTextureStrengthV1,
  withBrushTipAngleDegreesV1, withBrushTipHardnessV1, withBrushProceduralTipShapeV1,
  type BrushBehaviorV1, type BrushPresetV1,
...
export const DEFAULT_BRUSH_TIP_RESOURCE_EXTENSION_KEY_V1='illustro.defaultBrushTipResourceAlias' as const;
export const DEFAULT_BRUSH_PACK_CATEGORY_COUNTS_V1=Object.freeze({
  'Ink / Pen':8,Pencil:6,Marker:5,Paint:9,Airbrush:3,'Digital Watercolor':5,
  Eraser:4,'Blend / Smudge / Blur':3,'Scatter / Special':5,
} as const);
...
builtin.watercolor.round|Watercolor Round|Digital Watercolor|paint|38|0.34|0.18|SOM|p:builtin.grain.paper.01|-|0.52,0.36,0.54,0,0,0,0,0,0,4,0.35,1,0,0.48,0.48,0.2,0.88
builtin.watercolor.wash|Watercolor Wash|Digital Watercolor|paint|64|0.16|0.22|OM|p:builtin.grain.paper.02|-|0.34,0.24,0.62,0,0,0,0,0,0,4,0.35,1,0,0.6,0.36,0.3,0.94
builtin.watercolor.edge|Watercolor Edge|Digital Watercolor|paint|42|0.46|0.2|SM|p:builtin.grain.paper.05|-|0.48,0.3,0.58,0,0,0,0,0,0,4,0.35,1,0,0.44,0.42,0.18,0.84
builtin.watercolor.granulating|Watercolor Granulating|Digital Watercolor|paint|46|0.26|0.24|OM|g:builtin.grain.rough.04|-|0.46,0.28,0.8,0,0,0,0,0,0,4,0.35,1,0,0.52,0.4,0.26,0.9
builtin.watercolor.glaze|Watercolor Glaze|Digital Watercolor|paint|34|0.22|0.16|OM|p:builtin.grain.paper.09|-|0.3,0.2,0.42,0,0,0,0,0,0,4,0.35,1,0,0.36,0.28,0.16,0.96
...
function n(v:string):number{const x=Number(v);if(!Number.isFinite(x))throw new TypeError('invalid default brush numeric field');return x;}
function make(line:string):BrushPresetV1{
  const [id,name,category,behavior,sizeS,hardS,spaceS,flags,res,tip,...numbers]=line.split('|');
  if(!id||!name||!category||!behavior||!sizeS||!hardS||!spaceS||!flags||!res||!tip)throw new TypeError('invalid default brush row');
  const a=numbers.join('|').split(',').map(n);
  const [opacity=1,flow=1,texture=0,angle=0,stabilization=0,sizeJ=0,rotJ=0,posJ=0,densJ=0,
    particleDensity=4,particleSize=.35,spread=1,deviation=0,mixCanvas=.5,mixDeposit=.7,mixPickup=.15,mixCarry=.85]=a;
  let p=createBaselineBrushPresetV1({id,name,category,behavior:behavior as BrushBehaviorV1,defaultSizePx:n(sizeS),tags:[category,'Default Brush Pack']});
  ...
  p=withBrushTipHardnessV1(p,n(hardS));p=withBrushStrokeSpacingV1(p,n(spaceS));
  p=withBrushPressureSizeEnabledV1(p,flags.includes('S'));p=withBrushPressureOpacityEnabledV1(p,flags.includes('O'));
  if(angle!==0)p=withBrushTipAngleDegreesV1(p,angle);if(flags.includes('F'))p=withBrushFollowStrokeRotationV1(p,true);
  if(stabilization!==0)p=withBrushRealtimeStabilizationAmountV1(p,stabilization);
  if(res.startsWith('g:')){const x=res.slice(2);if(!grains.has(x))throw new RangeError(`unknown grain: ${x}`);p=withBrushGrainResourceIdV1(p,x);}
  if(res.startsWith('p:')){const x=res.slice(2);if(!papers.has(x))throw new RangeError(`unknown paper: ${x}`);p=withBrushPaperTextureResourceIdV1(p,x);}
  if(texture!==0)p=withBrushTextureStrengthV1(p,texture);
  if(tip!=='-'){if(!tipAliases.has(tip))throw new RangeError(`unknown tip: ${tip}`);p=normalizeBrushPresetV1({...p,extensions:{...p.extensions,[DEFAULT_BRUSH_TIP_RESOURCE_EXTENSION_KEY_V1]:tip}});}
  if(flags.includes('M')){p=withBrushColorMixEnabledV1(p,true);p=withBrushColorMixCanvasRatioV1(p,mixCanvas);
    p=withBrushColorMixDepositAmountV1(p,mixDeposit);p=withBrushColorMixPickupAmountV1(p,mixPickup);p=withBrushColorMixCarryAmountV1(p,mixCarry);}
  if(sizeJ!==0)p=withBrushSizeJitterV1(p,sizeJ);if(rotJ!==0)p=withBrushRotationJitterV1(p,rotJ);
  if(posJ!==0)p=withBrushPositionJitterV1(p,posJ);if(densJ!==0)p=withBrushDensityJitterV1(p,densJ);
  if(flags.includes('P')){p=withBrushSprayEnabledV1(p,true);p=withBrushSprayParticleDensityV1(p,particleDensity);
    p=withBrushSprayParticleSizeRatioV1(p,particleSize);p=withBrushSpraySpreadRadiusRatioV1(p,spread);p=withBrushSprayDeviationV1(p,deviation);}
  return normalizeBrushPresetV1(p);
}
const PACK=Object.freeze(RAW.split('\n').filter(Boolean).map(make));
function validate():void{
  if(PACK.length!==48||new Set(PACK.map(p=>p.id)).size!==48||new Set(PACK.map(p=>p.name)).size!==48)throw new TypeError('invalid 48-preset identity inventory');
  for(const [c,k] of Object.entries(DEFAULT_BRUSH_PACK_CATEGORY_COUNTS_V1))if(PACK.filter(p=>p.category===c).length!==k)throw new RangeError(`category mismatch: ${c}`);
  for(const p of PACK)if(p.schema!==BRUSH_V1_SCHEMA)throw new TypeError('default brush schema mismatch');
  const clean=new Set(['G Pen','Round Pen','Mapping Pen','Technical Pen','Brush Pen','Hard Airbrush','Soft Airbrush','Hard Eraser','Soft Eraser','Precision Eraser']);
  for(const p of PACK)if(clean.has(p.name)&&(defaultBrushTipResourceAliasV1(p)!==null||p.texture.resourceId!==undefined))throw new TypeError(`clean procedural brush uses sampled data: ${p.name}`);
}
validate();
export function createDefaultBrushPackV1():readonly BrushPresetV1[]{return Object.freeze([...PACK]);}
export function defaultBrushPackCanonicalJsonV1():string{return `${JSON.stringify({schema:DEFAULT_BRUSH_PACK_SCHEMA_V1,regenerationId:DEFAULT_BRUSH_PACK_REGENERATION_ID_V1,presets:PACK})}\n`;}
